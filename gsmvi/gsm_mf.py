...
def _gsm_update_single(sample, v, mu0, S0):
    ...
    mu_vT = (mu0 - sample) * v
    den = 1 + rho + mu_v
    I = jnp.ones(sample.shape[0])
    mu_update = 1/(1 + rho) * ( I - mu_vT / den) * eps0
    mu = mu0 + mu_update

    #S update
    Supdate_0 =  (mu0-sample) **2 
    Supdate_1 =  (mu-sample) **2
        ...
        for i in range(niter + 1):
            ...
            if monitor is not None:
                if (i%monitor.checkpoint) == 0:
                    monitor(i, [mean, np.diag(cov)], self.lp, key, nevals=nevals)
                    nevals = 0
            ...
            samples = np.random.multivariate_normal(mean=mean, cov=np.diag(cov), size=batch_size)
