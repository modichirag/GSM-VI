...
class Pathfinder():
    ...
    def fit(self, key, x0=None, num_samples=200, maxiter=1000, return_path=False):
        """
        ...
        if return_path :
            try:
                finder = pathfinder(key, self.lp, num_samples=num_samples, max_iter=maxiter, return_path=True)
                path = finder.init(x0)
        ...
        finder = pathfinder(key, self.lp, num_samples=num_samples, max_iter=maxiter)
