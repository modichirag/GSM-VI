...
def backward_kl_gauss(samples, mu, cov, lp):
...
@partial(jit, static_argnums=(3))
def forward_kl_gauss(samples, mu, cov, lp):
    q = MultivariateNormal(mu, cov)
    logq = jnp.sum(q.log_prob(samples))
    logl = jnp.sum(lp(samples))
    fkl = logl- logq
...
class KLMonitor_Gauss():
    ...
    def __call__(self, i, params, lp, key, nevals=1):
        ...
        try:
            ...
            self.bkl.append(backward_kl_gauss(qsamples, mu, cov, lp))
            ...
            if self.ref_samples is not None:
                ...
                self.fkl.append(forward_kl_gauss(psamples, mu, cov, lp))
